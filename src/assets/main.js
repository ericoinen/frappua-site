...
void main(){
  ...
  float ripple=smoothstep(0.45,0.0,dist)*(0.010+0.022*u_hover);
  uv += normalize(dir+1e-4)*sin(dist*26.0-u_time*3.2)*ripple;
  uv += (n-0.5)*(0.005+u_vel*0.03);
  float ab=0.0022+u_vel*0.022+ripple*1.1+u_hover*0.003;
  vec3 col;
  col.r=texture2D(u_tex,uv+vec2(ab,0.0)).r;
  col.g=texture2D(u_tex,uv).g;
  col.b=texture2D(u_tex,uv-vec2(ab,0.0)).b;
  float lum=dot(col,vec3(0.299,0.587,0.114));
  col=mix(col,u_accent*pow(lum,0.9)*1.25,0.13);
  col+=u_accent*0.035;
  vec2 d=abs(vUv-0.5);
  float fx=smoothstep(0.5,0.40,d.x), fy=smoothstep(0.5,0.40,d.y);
